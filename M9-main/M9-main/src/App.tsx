...
const App: React.FC = () => {
    ...
    useEffect(() => {
        // This code only runs on a mobile device (iOS or Android)
        if (Capacitor.isNativePlatform()) {
            // Make the web view draw behind the status bar
            StatusBar.setOverlaysWebView({ overlay: true });
        }
    }, []);
    
    // Set status bar style based on current map layer for better visibility
    useEffect(() => {
        if (Capacitor.isNativePlatform()) {
            const style = currentLayer === 'dark' ? Style.Light : Style.Dark;
    ...
    const handleSearch = useCallback(async (query: string) => {
        ...
        try {
            const searchResult = await geocodeLocation(query);
            if (!searchResult) throw new Error(`Could not find location: ${query}`);

            setSearchedLocation(searchResult);
            setMapCenter([searchResult.lat, searchResult.lng]);
            setZoom(15);
        } catch (err: any) {
